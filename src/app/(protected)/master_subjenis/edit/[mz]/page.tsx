...
import FormEditPage from "@/components/masterSubjenis/formEditPage";
...
const EditPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await apiRequest(`/master_subjenis/${id}`, "GET");
        if (!response.ok) {
  ...
  const breadcrumbs = [
    { name: "Dashboard", href: "/" },
    { name: "Setting Subjenis", href: "/master_subjenis" },
