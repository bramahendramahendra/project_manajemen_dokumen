...
import FormEditPage from "@/components/masterPerihal/formEditPage";
...
const EditPage = () => {
  ...
  const [perihal, setPerihal] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const { id: decryptedID, perihal: decryptedPerihal } = result;
   
    setID(decryptedID);
    setPerihal(decryptedPerihal);
  }, [encrypted, user]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await apiRequest(`/master_perihal/${id}`, "GET");
        if (!response.ok) {
        ...
        <div className="col-span-12 xl:col-span-6">
          {loading ? (
            ...
            <div className="text-left text-red-500">
              Data untuk menu dengan perihal <strong>{perihal}</strong> tidak ada.
            </div>
          )}
        </div>
      </div>
