...
const ValidationUploadTable = ({ id }: Props) => {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error("Data validasi dokumen tidak ditemukan");
        }
  ...
  useEffect(() => {
    const allCheckedInPage = dataDetail.every((_, index) => checkedItems[(currentPage - 1) * itemsPerPage + index]);
    setIsAllChecked(allCheckedInPage);
  }, [currentPage, checkedItems, dataDetail, itemsPerPage]);
  ...
  const handleItemCheck = (index: number) => {
    ...
    setIsAllChecked(dataDetail.every((_, idx) => newCheckedItems[(currentPage - 1) * itemsPerPage + idx]));
  };
        ...
        <div className="max-w-full overflow-x-auto rounded-lg">
          <table className="w-full table-auto">
            ...
            <tbody>
              ...
              {!loading && !error && dataDetail.length > 0 && dataDetail.map((item, index) => (
                <tr key={item.id}
